            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <select class="form-control aiz-selectpicker" name="gender" required>
                        <option value="1" @if($member->member && $member->member->gender == 1) selected @endif>
                            {{translate('Male')}}</option>
                        <option value="2" @if($member->member && $member->member->gender == 2) selected @endif>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="aiz-date-range form-control" name="date_of_birth"
                        value="@if($member->member && !empty($member->member->birthday)) {{date('Y-m-d', strtotime($member->member->birthday))}} @endif"
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-control aiz-selectpicker" name="on_behalf" data-live-search="true" required>
                        @foreach ($on_behalves as $on_behalf)
                            <option value="{{$on_behalf->id}}" @if($member->member && $member->member->on_behalves_id == $on_behalf->id) selected @endif>{{$on_behalf->name}}
                            </option>
                        @endforeach
                    </select>
                    @error('on_behalf')
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <input type="text" name="children" value="{{ $member->member->children ?? '' }}"
                        class="form-control" placeholder="{{translate('Number Of Son')}}">
                </div>
                <div class="col-md-6">
                    <label for="first_name">{{translate('Number Of Daughter')}}
                        <span class="text-danger">*</span>
                    </label>
                    <input type="text" name="no_of_daughter" value="{{ $member->member->no_of_daughter ?? '' }}"
                        class="form-control" placeholder="{{translate('Number Of Daughter')}}">
                </div>
            </div>
